...
const publicThingDB = require('../Database/thingDatabase');
...
router.get('/publicThings', function(req, res, next) {
    ...
    publicThingDB.getAll((status, things) => {
        if (status == 200) {
...
router.get('/publicThings/:id', function(req, res, next) {
    publicThingDB.get(req.params.id, (status, thing) => {
        if (status == 200) {
...
router.post('/publicThings', function(req, res, next) {
    ...
    //Save thing
    publicThingDB.save(req.body.type, req.body.description, (status, savedThing) => {
...
router.put('/publicThings/:id', function(req, res, next) {
    ...
    publicThingDB.update(req.params.id, req.body.type, req.body.description, (status, updatedThing) => {
...
router.delete('/publicThings/:id', function(req, res, next) {
    publicThingDB.remove(req.params.id, (status, deletedThing) => {
        if (status == 200) {
